...
| defaultSettings.dataEngineLvolClearMethod | Applies only to the V2 Data Engine. Specifies the data clearing method ('none', 'unmap', or 'write_zeroes') used when a logical volume (lvol) is deleted from the SPDK logical volume store. |
| defaultSettings.dataEngineLvolThinProvision | Applies only to the V2 Data Engine. Specifies whether logical volumes (lvols) are thin-provisioned on the SPDK logical volume store. |
| defaultSettings.dataEngineLvstoreClusterSize | Applies only to the V2 Data Engine. Specifies the cluster size, in bytes, used when creating the SPDK logical volume store on a disk. Changing this setting does not affect existing logical volume stores. |
